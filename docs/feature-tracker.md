...
| BUG-014 | Bug | P1 | 🟦 in-progress | Auth/Session UX | Logout/login does not fully clear frontend transient state | Depends on: FEAT-005. Added centralized client-state reset on no-session/login/logout paths; finish with token-refresh failure path verification + final cleanup pass |
...
| BUG-019 | Bug | P1 | 🟩 done | Admin/Chat Scope UX | Admin panel channel suggestions were coupled to character-scoped chat channel reads | Decoupled admin panel channel-key source from character-scoped chat list so admin controls no longer inherit selected-character channel scope |
...
| FEAT-004 | Feature | P1 | 🟨 todo | Realms | Realm naming + selector UX | Depends on: MMO-001. Introduce realm metadata model and replace raw id labels once realm scoping defaults are finalized |
| FEAT-005 | Feature | P1 | 🟨 todo | Startup UX | First-load sequencing hardening | Depends on: MMO-001. Remove transient error flashes during boot after scoped bootstrap sequencing is stable |
